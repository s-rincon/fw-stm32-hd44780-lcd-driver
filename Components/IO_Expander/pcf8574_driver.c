...
bool pcf8574_init(pcf8574_driver_t *driver, pcf8574_driver_config_t *config) {
    ...
    if (!pcf8574_i2c_interface_init(driver->hw_instance)) {
    ...
    pcf8574_i2c_interface_deinit(driver->hw_instance);
    driver->hw_instance = NULL;
    return false;
}
...
bool pcf8574_deinit(pcf8574_driver_t *driver) {
    ...
    if (!pcf8574_i2c_interface_deinit(driver->hw_instance)) {
        return false;
    }
...
bool pcf8574_read_port(pcf8574_driver_t *driver, uint8_t *data) {
    ...
    if (pcf8574_i2c_interface_read(driver->hw_instance, read_address, data, 1, driver->i2c_timeout_ms)) {
...
bool pcf8574_write_port(pcf8574_driver_t *driver, uint8_t data) {
    ...
    if (pcf8574_i2c_interface_write(driver->hw_instance, write_address, &data, 1, driver->i2c_timeout_ms)) {
        driver->current_output = data;
        return true;
    }
